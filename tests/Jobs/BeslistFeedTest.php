...
/**
 * Class BeslistFeedTest
 * @package Daalder\Feeds\Tests\Jobs
 */
class BeslistFeedTest extends FeedsTestBase
{
    protected function setUp(): void
    {
        parent::setUp();
        $this->createBeslistTestProducts();
    }

    /** @test */
    public function it_can_generate_feed()
    {
        $this->generate_feed_basetest(BeslistFeed::class, Store::first());
    }
    ...
    public function it_contains_all_valid_products()
    {
        ...
        $this->assertDirectoryExists(storage_path('feeds/beslist'));

        // Get the filePath for this vendor/feed combination (it's suffixed with a random string)
        $filePath = $this->getFeedFilePath('beslist', $store->code);
